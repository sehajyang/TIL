...
소프트웨어 설계와 구현을 재사용 가능하도록 해주는 뼈대를 미리 구현한 클래스, 인터페이스, 메서드 등의 모음

구체적, 확장 가능한 기반코드를 가지고 설계자의 의도에 따르는 여러 디자인 패턴 집합으로 구성되어있다.
미리 만들어져있어 빨리 사용이 가능하며 사용하기 쉽지만 프레임워크 내부를 커스텀마이징하긴 힘들다.

* 라이브러리와의 차이점 
  라이브러리는 사용할 수 있는 함수들의 모음이고 프레임워크는 뼈대구조(채워넣을수 있는) 다.
